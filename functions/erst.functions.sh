...
kodestandard() {
    ...
    git clone git@rep.erst.dk:kerneb/kerneb-kodestandard.git "$tmpdir" --depth=1 --quiet --branch "$@" ||
    # otherwise clone default branch:
    git clone git@rep.erst.dk:kerneb/kerneb-kodestandard.git "$tmpdir" --depth=1 --quiet) &&
